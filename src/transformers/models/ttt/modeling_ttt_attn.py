...
class TttCache:
    def __init__(self, config, batch_size, dtype=torch.float32, device=None):
        self.seqlen_offset = torch.zeros(batch_size, dtype=torch.int64, device=device)
        self.dtype = dtype
    ...
    def update(self, py_tree, layer_idx, batch_seq_len):
        seqlen_offset = self.seqlen_offset.max().item()
        seq_len = batch_seq_len.max().item()
        if seq_len % self.inner_chunk_size == 0:
            ...
            if seq_len != 1 and seqlen_offset > 0 and seqlen_offset % self.inner_chunk_size != 0:
                raise ValueError("fractional update not supported yet.")
            if (seq_len + seqlen_offset) % self.inner_chunk_size == 0:
                for name in self.param_names:
                    self.params_pytree[f"{name}_states"][layer_idx].copy_(py_tree[f"{name}_states"])
                    self.params_pytree[f"{name}_grad"][layer_idx].zero_()
            else:
        ...
        # for batch_idx, seq_len in enumerate(batch_seq_len):
        #     seq_len = seq_len.item()
        #     seqlen_offset = self.seqlen_offset[batch_idx].item()
        #     if seq_len % self.inner_chunk_size == 0:
        #         for name in self.param_names:
        #             self.params_pytree[f'{name}_states'][layer_idx][batch_idx].copy_(py_tree[f'{name}_states'][batch_idx])
        #             self.params_pytree[f'{name}_grad'][layer_idx][batch_idx].zero_()
        ...
        #         else:
        #             for name in self.param_names:
        #                 self.params_pytree[f'{name}_grad'][layer_idx][batch_idx].copy_(py_tree[f'{name}_grad'][batch_idx])
        #     else:
        #         raise ValueError(f'seq_len {seq_len} is a partial update not supported yet')
        # elif seq_len < self.inner_chunk_size:
...
def batch_select(tensor, index):
    index = index.reshape((1,) * (tensor.ndim))
    index = index.expand((1,) + tensor.shape[1:])
    return torch.gather(tensor, 0, index)[0]


def find_last_max_index(tensor):
    # Find the maximum value in the tensor
    max_val = torch.max(tensor)

    # Create a boolean mask where the tensor equals the max value
    mask = tensor == max_val

    # Create a range tensor of the same shape as the input tensor
    indices = torch.arange(tensor.size(0), device=tensor.device)

    # Multiply the mask by the range, setting non-max elements to 0
    masked_indices = mask * indices

    # The maximum of masked_indices is the last occurrence of the max value
    max_index = torch.max(masked_indices)

    return max_index  # Convert to Python scalar
...
class TttModule(nn.Module):
    ...
    def forward_chunk(
        ...
        if cache_params is not None:
            ...
            inner_chunk_step_offset = torch.zeros(B, dtype=torch.int64, device=batch.device)

        n_chunk = L // inner_chunk_size
        if attention_mask is not None:
            attention_mask = attention_mask.reshape(B, n_chunk, inner_chunk_size)
        ...
        token_idx = self.token_idx[
            inner_chunk_step_offset.unsqueeze(-1) + torch.arange(inner_chunk_size, device=batch.device)
        ]

        coeff = (self.config.inner_net_lr * token_idx).reshape(B, 1, 1, -1, 1) * ilr_gated / self.head_dim

        @torch.vmap
        def update_embed(XA, XB, XC, coeff, attention_mask=None, last_params_pytree=None):
            ...
            def parallelize_over_heads(XA, XB, XC, coeff, init_params_pytree, ln_weight, ln_bias):
                def compute_chunk(params_pytree, inputs):
                    ...
                    # [inner_chunk_size]
                    attn_mask_chunk = inputs.get("attention_mask", None)
                    ...
                    last_valid_idx = torch.as_tensor(inner_chunk_size - 1, device=XA_chunk.device)
                    if attn_mask_chunk is not None:
                        grad_l_wrt_Z1 = grad_l_wrt_Z1 * attn_mask_chunk.unsqueeze(-1)
                        grad_l_wrt_Z2 = grad_l_wrt_Z2 * attn_mask_chunk.unsqueeze(-1)

                        # valid_indices = attn_mask_chunk.nonzero().squeeze(-1)
                        # if len(valid_indices) > 0:
                        #     last_valid_idx = attn_mask_chunk.nonzero().squeeze(-1)[-1]
                        # last_valid_idx = F.relu(torch.cumsum(attn_mask_chunk, dim=0).max() - 1)
                        # print('attention_mask', attn_mask_chunk.sum(), last_valid_idx)
                        # all grads are zero, last_valid_idx could be whatever is fine
                        last_valid_idx = find_last_max_index(attn_mask_chunk)
                    ...
                    if cache_params is not None and inner_chunk_size % self.inner_chunk_size != 0:
                        ...
                        #     'b2_grad': grad_b2[-1:],
                        # }
                        grad_W1 = batch_select(grad_W1, last_valid_idx)
                        grad_W2 = batch_select(grad_W2, last_valid_idx)
                        grad_b1 = batch_select(grad_b1, last_valid_idx)
                        grad_b2 = batch_select(grad_b2, last_valid_idx)
                        coeff_chunk = batch_select(coeff_chunk, last_valid_idx)

                        grad_pytree = {
                            "W1_grad": grad_W1,
                            "W2_grad": grad_W2,
                            "b1_grad": grad_b1.unsqueeze(0),
                            "b2_grad": grad_b2.unsqueeze(0),
                        }

                        W1_last = W1_init - (coeff_chunk * grad_W1)
                        W2_last = W2_init - (coeff_chunk * grad_W2)
                        b1_last = batch_select(b1_bar, last_valid_idx).unsqueeze(0)
                        b2_last = batch_select(b2_bar, last_valid_idx).unsqueeze(0)
                        ...
                        W1_last = W1_init - (coeff_chunk * X1).transpose(1, 0) @ grad_l_wrt_Z1
                        W2_last = W2_init - (coeff_chunk * X2).transpose(1, 0) @ grad_l_wrt_Z2
                        b1_last = batch_select(b1_bar, last_valid_idx).unsqueeze(0)
                        b2_last = batch_select(b2_bar, last_valid_idx).unsqueeze(0)
        ...
        if last_chunk_params_pytree is not None:
            XCW_batch, batch_params_pytree = update_embed(XA, XB, XC, coeff, attention_mask, last_chunk_params_pytree)
        else:
            XCW_batch, batch_params_pytree = update_embed(XA, XB, XC, coeff, attention_mask)
        # [B, num_heads, L, C]
        if cache_params is not None:
            batch_seq_len = attention_mask.sum(-1)
            # NOTE: use the same batch seqence length for now
            batch_seq_len = torch.ones_like(batch_seq_len) * L
            cache_params.update(batch_params_pytree, self.layer_idx, batch_seq_len)
...
class TttModel(TttPreTrainedModel):
    ...
    def forward(
        ...
        if use_cache:
            if attention_mask is not None:
                cache_params.seqlen_offset += attention_mask.sum(dim=-1)
            else:
...
class TttForCausalLM(TttPreTrainedModel):
    ...
    def forward(
        ...
        *,
        ...
        print("input_ids", input_ids.shape, "time", time.time() - start_time)
